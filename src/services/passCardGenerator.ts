...
export function generatePassCard(params: {
  ...
  ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(0, 4); ctx.lineTo(width, 4); ctx.stroke();
  ...
  ctx.lineWidth = 1;
  ctx.beginPath(); ctx.moveTo(0, height - 52); ctx.lineTo(width, height - 52); ctx.stroke();
