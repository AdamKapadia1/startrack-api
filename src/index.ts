...
let issTleData: { name: string; line1: string; line2: string } | null = null;

async function fetchIssTle(): Promise<void> {
  try {
...
fetchIssTle();
setInterval(fetchIssTle, 6 * 60 * 60 * 1_000);
...
app.get('/api/insights/heatmap', async (req, res) => {
  if (!supabase) { res.json({ days: [] }); return; }
  try {
    ...
    if (!data || data.length === 0) { res.json({ days: [] }); return; }
...
async function logTopPassesForValidation() {
  if (!supabase) { console.log('[validation] Supabase not configured — skipping daily auto-log'); return; }
  try {
    await getActiveSatellites(1);
    const tles  = getAllSatellites();
    const passes = predictPasses(tles, DEFAULT_LAT, DEFAULT_LON, DEFAULT_ALT_M / 1000, 1, 60, 0);
...
app.post('/api/validation/record-actual', async (req: Request, res: Response) => {
  if (!supabase) { res.status(503).json({ error: 'Supabase not configured' }); return; }
  try {
    const { id, actualPeakTime, actualMaxElevation, actualAzimuth } = req.body as {
      id: number; actualPeakTime: string; actualMaxElevation: number; actualAzimuth?: number;
    };
...
app.get('/api/iss/photo', async (_req: Request, res: Response) => {
  ...
  const imageUrl: string | null = items.find((i: any) =>
    i.href?.includes('large') || i.href?.includes('orig'),
  )?.href ?? null;
...
app.get('/api/iss/info', async (req: Request, res: Response) => {
  try {
    ...
    let smaKm: number | null = null, apogeeKm: number | null = null, perigeeKm: number | null = null, periodMin: number | null = null;
