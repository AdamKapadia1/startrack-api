...
app.get('/api/debug/tle-status', async (_req, res) => {
    try {
        await (0, tleCache_js_1.getActiveSatellites)(50);
        const status = (0, tleCache_js_1.getTleStatus)();
        res.json({
            ...status,
...
function checkSunlit(pos, sunDir) {
    const Re = 6378.137;
...
function calcBetaAngle(pos, vel, sunDir) {
...
function getLookAt(satrec, t, obsGd) {
    const pv = satelliteJs.propagate(satrec, t);
    if (!pv || !pv.position || typeof pv.position === 'boolean')
        return null;
    const gmst = satelliteJs.gstime(t);
    const posEcf = satelliteJs.eciToEcf(pv.position, gmst);
    const la = satelliteJs.ecfToLookAngles(obsGd, posEcf);
    return { az: la.azimuth * (180 / Math.PI), el: la.elevation * (180 / Math.PI), range: la.rangeSat };
}
function getRangeRate(satrec, t, obsGd) {
    const DT = 5000;
    ...
    try {
        ...
        if (issTleData) {
            try {
                ...
                tleAgeHours = (now.getTime() - epochDate.getTime()) / 3600000;
                ...
                if (pv && pv.position && typeof pv.position !== 'boolean' &&
                    ...
                    for (let i = 1; i <= 720; i++) {
                        ...
                        if (!pvS || !pvS.position || typeof pvS.position === 'boolean')
                            continue;
                        const sunlitS = checkSunlit(pvS.position, getSunDirECI(dateToJD(tS)));
                        if (sunlitS !== sunlit) {
                            timeToTransitionSecs = i * 60;
                            break;
                        }
                    }
                }
                ...
                passDetails = passes.slice(0, 3).map(p => {
                    ...
                    const dkHz = (rr) => Math.round(-(VHF_HZ * rr / C_KMS) / 10) / 100;
                    return {
                        startUTC: p.startUTC,
                        maxUTC: p.maxUTC,
                        endUTC: p.endUTC,
        ...
        const crew = crewRes
            ? crewRes.data.people.filter(p => p.craft === 'ISS').map(p => p.name)
            : [];
